<x-main-layout>
    ...
    <div class="max-w-3xl mx-auto w-full pb-20" 
                ...
                @foreach($tweets as $tweet)
                    <div class="p-6 border-b border-gray-200 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-900/40 transition cursor-pointer">
                        <div class="flex gap-4">
                            ...
                            <div class="flex-1 min-w-0"> <!-- min-w-0 fix for truncation -->
                                <div class="flex justify-between items-start">
                                    ...
                                    <div class="relative" x-data="{ open: false }">
                                        ...
                                        <div x-show="open" @click.away="open = false" class="absolute right-0 mt-2 w-32 bg-white dark:bg-gray-900 rounded-xl shadow-[0_0_15px_rgba(0,0,0,0.1)] border border-gray-100 dark:border-gray-800 z-10 overflow-hidden" x-cloak>
                                            <button @click="editModalOpen = true; editRoute = '{{ route('tweets.update', $tweet) }}'; editContent = '{{ addslashes($tweet->content) }}'; open = false" class="w-full text-left px-4 py-3 text-black dark:text-white font-bold hover:bg-gray-50 dark:hover:bg-gray-800 text-sm border-b border-gray-100 dark:border-gray-800">Edit</button>
                                            <button @click="deleteModalOpen = true; deleteRoute = '{{ route('tweets.destroy', $tweet) }}'; open = false" class="w-full text-left px-4 py-3 text-red-600 font-bold hover:bg-gray-50 dark:hover:bg-gray-800 text-sm">Delete</button>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="mt-3 flex items-center gap-4">
                                    <div x-data="{ liked: {{ $tweet->isLikedBy(Auth::user()) ? 'true' : 'false' }}, count: {{ $tweet->likes_count }} }">
                                        <button @click="liked = !liked; count = liked ? count + 1 : count - 1; @if(Auth::id() === $tweet->user_id) $dispatch('update-likes', { amount: liked ? 1 : -1 }); @endif fetch('{{ route('tweets.like', $tweet) }}', { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': '{{ csrf_token() }}' } });" class="group flex items-center gap-1.5 transition" :class="liked ? 'text-purple-600' : 'text-black dark:text-white'">
        ...
        <!-- CONTENT: THOUGHTS LIKED -->
        <div x-show="tab === 'liked'" style="display: none;">
            @if($likedTweets->count() > 0)
                @foreach($likedTweets as $tweet)
                    @if($tweet)
                    <div x-data="{ liked: true, count: {{ $tweet->likes_count ?? 0 }} }" x-show="liked" x-transition.duration.300ms class="p-6 border-b border-gray-200 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-900/40 transition">
                        <div class="flex gap-4">
                            <div class="flex-shrink-0">
                                <div class="w-10 h-10 rounded-full bg-gray-200 dark:bg-gray-800 overflow-hidden border border-gray-100 dark:border-gray-800">
                                     @if($tweet->user->profile_photo_path)
                ...
                <form :action="editRoute" method="POST">
                    ...
                    <div class="flex gap-4 mb-2">
                        ...
                        <div class="flex-1">
                            ...
                            <textarea name="content" rows="4" class="w-full bg-transparent text-lg placeholder-gray-400 text-black dark:text-white p-0 resize-none border-0 focus:border-0 focus:ring-0 shadow-none outline-none ring-0" x-model="editContent" maxlength="280"></textarea>
